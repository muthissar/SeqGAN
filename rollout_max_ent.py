...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate, normalize=True):
        ...
        # THESE TWO METHODS WORKS LIKE THIS:
        #   1. Make a batch of rollouts. 
        #   2. For each token in sequence make new rollouts, that corresponds to the rest of the seq
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator, ent_temp = 1.0):
        ...
        for i in range(rollout_num):
            ...
            #TODO: is there any stochastics here???
            #SOL. No this is probably also a bug. It always gives the same . i.e. 
            # ypred_for_auc - sess.run(discriminator.ypred_for_auc, feed) = 0
            feed = {discriminator.input_x: input_x, discriminator.dropout_keep_prob: 1.0}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
